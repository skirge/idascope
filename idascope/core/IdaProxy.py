...
###############################################################################
    ...
    def AddHotkey(self, hotkey, function):
        return self.ida_kernwin.add_idc_hotkey(hotkey, function)
    ...
    def DelSeg(self, address, flags):
        return ida_segment.del_segm(address, flags)
    ...
    def GetFunctionFlags(self, ea):
        return self.idc.get_func_attr(ea, idc.FUNCATTR_FLAGS)

    def GetFunctionName(self, address):
        return self.idc.get_func_name(address)

    def GetFrame(self, ea):
        return self.idc.get_func_attr(ea, FUNCATTR_FRAME)
    ...
    def MakeRptCmt(self, ea, cmt):
        return self.idc.set_cmt(ea, cmt, 1)
    ...
    def GetOpnd(self, ea, n):
        return self.idc.get_operand(ea, n)
...
###############################################################################
    ...
    def CompileLine(self, line):
        return self.ida_expr.compile_idc_text(line)
